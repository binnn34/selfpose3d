...
class Cutout:
    ...
    def __call__(self, pil_img, size=None):
        pil_img = pil_img.copy()
        img_height, img_width, num_channels = (*pil_img.size, 3)
        _, upper_coord, lower_coord = self._create_cutout_mask(
            img_height, img_width, num_channels, self.size if size is None else size
        )
...
class RandAugment:
    ...
    def get_trans_list(cls):
        ...
        # trans_list = ['Sharpness', 'AutoContrast', 'Posterize', 'Equalize', 'Contrast', 'Color', 'Brightness']

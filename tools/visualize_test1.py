...
_tmp_plotter = Plotter(offscreen=True)
_tmp_plotter.camera.SetPosition(4000.0, 2500.0, 4000.0)
_tmp_plotter.camera.SetFocalPoint(0.0, 10.0, 1000.0)
_tmp_plotter.camera.SetViewUp(0.0, 0.0, 1.0)
VIRTUAL_CAM2 = _tmp_plotter.camera
...
def clone_camera(camera):
    ...
    new_cam = plotter.camera
    new_cam.SetPosition(camera.GetPosition())
    new_cam.SetFocalPoint(camera.GetFocalPoint())
    new_cam.SetViewUp(camera.GetViewUp())
    new_cam.SetClippingRange(camera.GetClippingRange())
    return new_cam
...
def render_sequence(SEQUENCE_NAME):
    ...
    for i in tqdm(range(num_images), desc="rendering results"):
        ...
        if RENDER_TO_FILE:
            ...
            camera_copy = clone_camera(VIRTUAL_CAM2)

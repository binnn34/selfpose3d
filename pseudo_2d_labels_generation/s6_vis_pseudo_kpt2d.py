...
random.seed(0)
GT_FILE = "./pseudo_labels/pseudo_kpt2d_panoptic.json"
IMGDIR = "./data"
...
def main():
    ...
    for k in range(len(list(dets))):
        #k = random.choice()
        anns = dets[k]
        file_name = os.path.join(IMGDIR, id2filename[k])
        if "160422_ultimatum1" in file_name and "00017069" in file_name:
            img = cv2.imread(file_name)
            anns = sorted(anns, key=lambda k: k["bbox"][0])
            img = draw_anns_v2(img, anns, rand_colors)
            cv2.imshow("img", img)
            cv2.imwrite("test/{}".format(os.path.basename(file_name)), img)
            ch = cv2.waitKey(0)
            if ch == 27:
                break
